...
class NucleiActivities:
    ...
    @activity.defn
    async def check_latest_cve_changes(self, targets: list[str], arguments: str): ...

    @activity.defn
    async def run_nuclei(self, targets: list[str], arguments: str) -> NmapResults: ...

    @activity.defn
    async def parse_nuclei_results(self): ...

    @activity.defn
    async def update_cve_lifecycle_info(self): ...

    def get_activities(self) -> Sequence[Callable[..., Awaitable[Any]]]:
        return [self.get_targets, self.check_latest_cve_changes, self.run_nuclei, self.parse_nuclei_results, self.update_cve_lifecycle_info]
