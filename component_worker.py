...
from temporalio.client import Client, Schedule, ScheduleActionStartWorkflow, ScheduleIntervalSpec, ScheduleSpec, ScheduleState, ScheduleAlreadyRunningError
...
def save_component_config(config):
    ...
    try:
        os.makedirs(os.path.dirname(COMPONENT_CONFIG_PATH), exist_ok=True)
        ...
        if "base-risk" not in autos:
            autos["base-risk"] = {
                ...
                "components": [
                    ...
                    {"name": "Threat",      "neo4jProperty": "threatScore", "weight": 0.333, "max_value": 10, "current_value": 0, "type": "threat"},
                ],
                "target_type": "all",
                "target_values": [],
...
async def initialize_core_component_schedules(client: Client):
    ...
    for component in core_components:
        ...
        schedule = Schedule(
            ...
            spec=ScheduleSpec(
                intervals=[ScheduleIntervalSpec(every=component['interval'])]
            ),
            state=ScheduleState(
...
async def initialize_base_risk_schedule(client: Client):
    """
    ...
    schedule = Schedule(
        ...
        spec=spec,
        state=ScheduleState(
            note="Base Risk (Criticality + CVSS + Threat) every 2h, offset 30m",
            paused=False
        )
    )
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                logger.error("Max retries reached. Could not connect to Temporal server.")
                raise
