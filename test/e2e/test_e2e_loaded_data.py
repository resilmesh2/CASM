...
class TestE2ELoadedData:
    def _fetch_payload(self, path: str) -> Any:
    ...
    def test_e2e_asset_info_snapshot(self, snapshot: SnapshotAssertion) -> None:
        """
        ...
        """
        payload = self._fetch_payload("/asset_info")
        assert payload == snapshot(name="asset_info")


    def test_e2e_cves_snapshot(self, snapshot: SnapshotAssertion) -> None:
        """
        ...
        Update snapshots intentionally via:
          E2E_API_SNAPSHOT=1 poetry run pytest test/e2e/test_api_snapshots.py --snapshot-update
        """
        payload = self._fetch_payload("/cves")
